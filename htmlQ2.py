      ...
      if firstparagraph==0:
        if "#" not in data[indexOfq]:
          if quizseperetor not in data[indexOfq]:
            write_file.write(f"<p style=\"margin-top:80px; \">{q}")
            firstparagraph+=1
            
        if "#" in data[indexOfq]:
          write_file.write(f"<p style=\"margin-top:80px; \">{q}</p>")
          firstparagraph+=1
